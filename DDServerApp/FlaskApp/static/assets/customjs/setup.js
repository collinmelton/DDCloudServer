...
function submitForm(event, submitType, editType, reload, fileid) {
	...
	// console.log($($(event.target).parent()[0]));
	// var formContents = $($(event.target).parent()[0]).serializeArray();
	// console.log(formContents);
	// formContents[formContents.length] = {"name": "submitType", value:submitType};
	formData.append("submitType", submitType);
	// formContents[formContents.length+1] = {"name": "editType", value:editType};
	formData.append("editType", editType);
	// console.log(formContents);
	// formContents
	$.ajax({
		url: getBaseUrl() + 'api/_workfloweditor',
		data: formData,
		processData: false,
		contentType: false,
		type: 'POST',
